...
export default function ShoppingTab({ state }) {
  const { plan, checked, updateChecked, prefs, isDemo } = state
    ...
    <section className="sec on">
      <div className="pad">
        <div className="empty-v">
          ...
          <div className="empty-s">Generate your meal plan first.</div>
        </div>
      </div>
    </section>
  )
  ...
  let total = 0
  list.forEach(cat => (cat.items || []).forEach(i => { total += (i.estimatedCost || 0) }))
      ...
      <div className="pad">
        ...
        <div className="bcard fu">
          <div className="bc-row">
            <div><div className="bc-lbl">Shopping total</div><div className="bc-big">{cur}{total.toFixed(2)}</div></div>
            <div className="bc-right"><div className="bc-rem">{cur}{Math.abs(rem).toFixed(2)}</div><div className="bc-remlbl">{rem>=0?'remaining':'over budget'}</div></div>
          </div>
        ...
        <div className="legend"><div className="mdot"></div>Green dot = used in multiple meals</div>
