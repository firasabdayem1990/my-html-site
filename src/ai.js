...
export async function fetchRecipe({ name, cuisine, desc, people, diet, restrictions, country, currency }) {
  const prompt = `Write a detailed authentic recipe for: "${name}"${cuisine ? ` (${cuisine})` : ''}${desc ? ` — ${desc}` : ''}.`
    ...
    + ` Return JSON only, no markdown: {"prepTime":"","cookTime":"","difficulty":"","calories":0,"protein":0,"carbs":0,"fat":0,"fiber":0,"pricePerServing":0,"ingredients":[{"qty":"","name":"","note":""}],"steps":[""],"tip":""}.`
    + ` calories, protein, carbs, fat, fiber = integers per ONE adult serving. protein/carbs/fat/fiber in grams.`
    + ` pricePerServing = realistic cost in ${currency} for ONE serving in ${country}.`
...
export async function searchRecipe({ query, people, diet, restrictions, country, currency }) {
  const prompt = `Recipe: "${query}". Serves ${people}. Diet: ${diet}. Restrictions: ${restrictions || 'none'}. Country: ${country}. Currency: ${currency}.`
    + ` Return JSON only: {"dishName":"","cuisine":"","prepTime":"","cookTime":"","difficulty":"","servings":${people},"pricePerServing":0,"calories":0,"protein":0,"carbs":0,"fat":0,"fiber":0,"ingredients":[{"qty":"","name":"","note":""}],"steps":[""],"tip":"","funFact":""}.`
    + ` calories, protein, carbs, fat, fiber = integers per ONE adult serving. protein/carbs/fat/fiber in grams.`
    + ` pricePerServing = realistic TOTAL cost in ${currency} to buy ALL ingredients from scratch in ${country}.`
